...
export class MetadataListComponent implements OnInit {
  ...
  createMetadata(): void {
    this.selectedMetadata = { id: 0, dataId:this.dataId ?? 0, value1: '', value2: '', fileName: '' };
  ...
  deleteMetadata(id: number): void {
    ...
    onFormSubmit(metadata: { id: number, value1: string, value2: string }): void {
      if (this.selectedMetadata?.id) {
        ...
        const newMetadata: Metadata = { id: 0, dataId: this.dataId ?? 0, value1: metadata.value1 , value2: metadata.value2, fileName: '' };
